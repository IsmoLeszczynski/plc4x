 ...
 */
public class ModbusAsciiConnection extends PollingSubscriptionConnectionBase<ModbusAsciiConfiguration> {
    ...
    protected void onConnect() throws PlcConnectionException {
        ...
        LOGGER.info("Modbus ASCII connection established");
        if (auditLog.isEnabled()) {
            auditLog.write(AuditLogEventType.CONNECT, "Modbus ASCII connection established");
        }
        ...
        try {
            if (auditLog.isEnabled()) {
                auditLog.write(AuditLogEventType.OUTGOING_MESSAGE,
                    "Sending Modbus ASCII request, address=" + address);
            }
    ...
    protected CompletableFuture<PlcPingResponse> onPing(PlcPingRequest pingRequest) {
        ...
        ModbusAsciiADU modbusAsciiADU = new ModbusAsciiADU(unitId, readRequestPdu);
        return executeThrottled(() ->
            sendRequest(modbusAsciiADU, unitId).thenApply(response ->
    ...
    private CompletableFuture<Map<String, PlcResponseItem<PlcValue>>> executeOptimizedRead(
        ...
        return executeThrottled(() ->
            sendRequest(modbusAsciiADU, unitId).thenApply(responseAdu -> {
